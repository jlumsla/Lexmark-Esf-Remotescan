...
public class RequestUISessionIdServlet extends HttpServlet
{
   // Eclipse gives a warning if this is not there
   private static final long serialVersionUID = 1L;

   public RequestUISessionIdServlet()
   {
   }
   ...
   {
      System.out.println("Executing RequestUISessionIdServlet");
      // Parse inputs
      String timeout = request.getParameter("RemoteScanTimeOut");
      System.out.println(timeout);
      String res = request.getParameter("res");
      System.out.println(res);

      // Generate a unique UISessionId and write to persistent storage
      ...
      writer.append("<result xmlns=\"urn:schemas-sc-jp:mfp:osa-1-1\" product-family=\"133\" product-version=\"x034Z20d>");

      writer.append("<RequestUISessonIdResponse>");
      writer.append("<uiSessionId>").append("SN6511928100UI040030558").append("</uiSessionId>");
      writer.append("</RequestUISessionIdResponse>");
         ...
         HttpServletResponse response) throws ServletException, IOException
   {
      ...
      Locale locale = getLocaleHelper(request);
      Messages messages = new Messages("Resources", locale,
            getClass().getClassLoader());

      out.println("<html><body>");
      out.println(
            "<p><b>" + messages.getString("httpservlet.message") + "</b>");
